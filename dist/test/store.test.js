...
class AsyncStore {
  ...
  set(sid, val, ttl) {
    ...
    return _asyncToGenerator(function* () {
      yield _this2.store.set(sid, val, ttl);
    })();
  }
  ...
  destroy(sid) {
    ...
    return _asyncToGenerator(function* () {
      yield _this3.store.destroy(sid);
    })();
  }
}
...
const test = (name, sessionStore) => {
  describe(`store adapter wraps ${name}`, () => {
    ...
    it('can set a session with ttl', done => {
      store.set(sid, data, ttl).then(done).catch(done);
    });

    it('will get "undefined" for a non-exist session', done => {
      store.get(sid).then(ret => {
    ...
    it('can destory a session', done => {
      store.destroy(sid).then(done).catch(done);
    });
